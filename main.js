...
let deckInfoDiv = document.createElement('div');
deckInfoDiv.id = "deckInfo";
deckInfoDiv.style.backgroundColor = "#34495e"; deckInfoDiv.style.padding = "10px 20px"; deckInfoDiv.style.borderRadius = "8px"; deckInfoDiv.style.margin = "10px 0";
deckInfoDiv.style.display = "flex"; deckInfoDiv.style.justifyContent = "space-around"; deckInfoDiv.style.fontWeight = "bold"; deckInfoDiv.style.border = "2px solid #7f8c8d"; deckInfoDiv.style.fontSize = "1.2em";
deckInfoDiv.innerHTML = `<span>📚 山札: <span id="deckCount">0</span> 枚</span><span>🗑️ 捨て札: <span id="discardCount">0</span> 枚</span>`;
statusBar.parentNode.insertBefore(deckInfoDiv, statusBar.nextSibling);
...
function updateDamagePreview() {
    let base = playerDamage * damageMultipliers.all;
    let rockDmg = base * damageMultipliers["グー"]; let scissorsDmg = base * damageMultipliers["チョキ"]; let paperDmg = base * damageMultipliers["パー"];
...
function initDeck() {
    ...
    for(let i=0; i<7; i++) { deck.push({ ...cardTemplates["グーUP"] }); deck.push({ ...cardTemplates["チョキUP"] }); deck.push({ ...cardTemplates["パーUP"] }); }
    for(let i=0; i<4; i++) { deck.push({ ...cardTemplates["ダブル"] }); deck.push({ ...cardTemplates["リドロー"] }); }
    for(let i=0; i<2; i++) { deck.push({ ...cardTemplates["グーパン"] }); deck.push({ ...cardTemplates["目つぶし"] }); deck.push({ ...cardTemplates["ビンタ"] }); }
    deck.push({ ...cardTemplates["グパ"] }); deck.push({ ...cardTemplates["グチョ"] }); deck.push({ ...cardTemplates["チョパ"] }); deck.push({ ...cardTemplates["救済"] });
    shuffleDeck();
}
...
function spawnDamageEffect(damageAmount) {
    ...
    let lifeDisplay = document.getElementById("lifeDisplay"); let rect = lifeDisplay.getBoundingClientRect();
    damageText.style.left = (rect.right - 80) + "px"; damageText.style.top = (rect.top + window.scrollY) + "px"; document.body.appendChild(damageText);
...
function showRewardScreen() {
    ...
    // ★ドラフト画面でも通常カードを出やすくする（完全ランダムから少しだけ調整）
...
function generateShop() {
    ...
    for(let i=0; i<6; i++) {
        // ★各枠について、15%の確率でレアカード抽選枠にする（85%は通常カード）
...
function buyShopBuff() {
    if (playerGold >= 1000) {
        ...
        if(currentShopBuff.type === "life") { playerLife += 2; updateLifeDisplay(); } 
...
function updateLifeDisplay() { document.getElementById("lifeDisplay").innerText = "❤️ あなた: " + playerLife + "  /  😈 敵: " + Math.max(0, enemyLife); }

function useCard(index) {
    let card = hand.at(index); se.card();
    ...
    } else if (card.type === "ダブル") {
        damageMultipliers.all *= 2; 
    } else if (card.type === "グーパン") {
        damageMultipliers["グー"] *= 3; 
    } else if (card.type === "目つぶし") {
        damageMultipliers["チョキ"] *= 3; 
    } else if (card.type === "ビンタ") {
        damageMultipliers["パー"] *= 3; 
    } else {
        changeProb(card.type, 10); updateProbDisplay();
    }
...
function playGame(playerHand) {
    ...
    if (isWin === true) {
        let finalDamage = playerDamage * damageMultipliers.all; 
